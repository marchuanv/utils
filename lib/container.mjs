...
export class Container {
    ...
    */
    static async validate(classInterfaceFilePath, targetClass) {
        ...
        await schema.validate(classInterfaceConfig);
        classInterfaceConfig.targetClass = targetClass;
    ...
    */
    static register(targetClass) {
        const config = configurations.find(config => config.targetClass === targetClass);
        if (config) {
            if (!privateBag.has(targetClass)) {
            ...
            throw new Error(`failed to register class: ${targetClass.name}`);
        }
    }
    /**
     * @template T
