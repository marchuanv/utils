...
export class Container extends Schema {
    ...
    */
    static register(classInterfaceFilePath, targetClass) {
        ...
        if (!privateBag.has(targetClass)) {
            TypeMapper.register(new GUID(), targetClass, false);
            const configStr = readFileSync(classInterfaceFilePath, 'utf8');
            let classIntefaceConfig = JSON.parse(configStr);
            const configKey = Object.keys(classIntefaceConfig)[0];
            classIntefaceConfig = classIntefaceConfig[configKey];
            if (classIntefaceConfig.extend && classIntefaceConfig.extend.Id) {
                const found = configurations.find(otherClassConfig => {
                    return otherClassConfig.Id === classIntefaceConfig.extend.Id
                });
                const otherKey = Object.keys(found)[0];
                classIntefaceConfig.extend = found[otherKey];
            }
            configurations.push(classIntefaceConfig);
            const classInterface = new ClassInterface(classIntefaceConfig);
