...
export class Bag extends UUID {
    ...
    */
    constructor(Id, secureContext) {
        if (!Reflection.isTypeOf(Id, String) || Reflection.isEmptyString(Id)) {
            throw new Error(`The Id argument is not a valid string.`);
        }
        if (!Reflection.isTypeOf(secureContext, SecureContext)) {
            throw new Error(`The secureContext argument is not an instance of ${SecureContext.name}.`);
        }
        super(Id);
    ...
    */
    static getProperty(bag, secureContext, property) {
        ...
        if (!Reflection.isTypeOf(property, Object) || Object.keys(property).length === 0) {
            throw new Error(`The property argument is not a valid object.`);
        }
        const properties = privateBag.get(secureBagKey.propertiesKey);
        const propertyName = Object.keys(property)[0];
