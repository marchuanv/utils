        ...
        if (isFuncValid) {
            ...
            try {
                ...
                for (const link of jsTypeMap.link) {
                    mappings.push(new JSTypeMap(new UUID(link)));
                }
            } catch (error) {
                console.log(error);
            }
    ...
    */
    static *get(func) {
        ...
        if (isFuncValid) {
            const jsTypeMap = map([func]);
            for (const link of jsTypeMap.link) {
                yield new JSTypeMap(new UUID(link));
            }
        } else {
...
function map(functions, defaultValue = null, isGenericType = null) {
    ...
    for (const otherFunc of functions) {
        ...
        if (!jsType.link.some(IdStr => IdStr === _Id.toString())) {
            jsType.link.push(_Id.toString());
        }
        const _jsType = new JSTypeMap(_Id, otherFunc, defaultValue, isGenericType);
        if (!_jsType.link.some(IdStr => IdStr === Id.toString())) {
            _jsType.link.push(Id.toString());
        }
    }
    return jsType
}
 ...
 * @param { Array<Function> } functions
*/
function unmap(functions) {
    let func = functions.shift();
    let functionName = typeof func === 'function' ? func.name : func;
    let jsType = new JSTypeMap(new UUID(functionName));
    for (const link of jsType.link) {
        const _jsType = new JSTypeMap(new UUID(link));
        const linkIndex = _jsType.link.findIndex(IdStr => IdStr === jsType.Id);
        if (linkIndex > -1) {
            _jsType.link.splice(linkIndex, 1);
        }
    }
    jsType.link.splice(0, jsType.link.length);
}