...
export class JSType extends Bag {
    ...
    */
    hasAtt(attType = null, func = null) {
        ...
        let attributeIds = this.getProperty(secureContext, { attributeIds: null });
        if (attType) {
            attributeIds = attributeIds.map(attId => this.getAttribute(secureContext, attId)).filter(att => att.attributeType === attType);
        }
        if (func) {
            attributeIds = attributeIds.filter(att => att.attributeType === attType && att.func === func);
        }
