...
function ensureMappedType(functions, defaultValue = null, isGenericType = false) {
    ...
    if (functions.length === 1) {
        ...
        let jsTypeA = new JSType(typeAId, mapping, funcA, defaultValue, isGenericType);
        let jsTypeB = new JSType(typeBId, mapping, funcB, defaultValue, isGenericType);
        functionName = typeof funcB === 'function' ? funcB.name : funcB;
        mapping = new JSTypeMapping(functionName);
        jsTypeA = new JSType(typeAId, mapping, funcA, defaultValue, isGenericType);
        jsTypeB = new JSType(typeBId, mapping, funcB, defaultValue, isGenericType);
    }
}