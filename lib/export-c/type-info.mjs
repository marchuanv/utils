    ...
    */
    constructor(targetType, isStatic = false) {
        ...
        for (const schemaMemberDescriptor of schemaMemberDescriptors) {
            ...
            const isMemberTypeInfo = schemaMemberDescriptor.method.toString().indexOf(`return new ${TypeInfoSchema.name}(`) > -1;
            if (!isMemberTypeInfo) {
                throw new Error(`${targetClass.name} ${key} member does not return ${TypeInfoSchema.name}.`);
            }
        }
