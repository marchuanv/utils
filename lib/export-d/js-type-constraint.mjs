...
export class JSTypeConstraint extends JSType {
    ...
    */
    constructor(func, constraint = null) {
        ...
        super(func);
        if (constraint) {
            if (!this.hasAtt(constraint, JSTypeConstraintAtt)) {
                this.setAtt(constraint, JSTypeConstraintAtt);
            }
        }
        if (this.hasAtt()) {
            let attributes = this.getAtt();
            attributes = Array.from(attributes);
            attributes = attributes.filter(att => att.attributeType === JSTypeConstraintAtt);
