    ...
    */
    constructor($interface) {
        ...
        if ($interface === null || $interface === undefined || !($interface instanceof Interface)) {
            throw new Error(`The $interface argument is null, undefined or not an instance of an ${Interface.name}`);
        }
        const { targetInterface, members, targetClasses } = $interface;
        super(`Id: 36d1d496-6b4a-4616-ab97-33d367385fa2, Interface: ${targetInterface.name}`, secureContext);
        if (this.hasState(secureContext, State.CONSTRUCT)) {
            if (!targetClasses.some(Class => Class === targetClass)) {
                throw new Error(`${targetClass.name} is not associated with the ${targetInterface.name} interface.`);
            }
