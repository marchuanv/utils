...
import { Serialiser, Bag, sha1 } from '../registry.mjs';
...
export class GUID {
    ...
    */
    constructor(metadata = null) {
        ...
        if (metadataType === 'object' && metadata !== null) {
            const json = Serialiser.serialise(metadata);
            IdStr = createUuid(json);
            if (!General.validateUuid(IdStr)) {
                throw new Error('guid generated from metadata is not valid.');
            }
        } else {
            if (General.validateUuid(metadata)) {
                IdStr = metadata;
            } else {
        ...
        if (Bag.has(IdStr)) {
            return Bag.get(IdStr, targetClass);
        } else {
            Bag.set(this, targetClass, IdStr);
        }
    }
    /**
    ...
    */
    destroy() {
        const IdStr = privateBag.get(this);
        guids.delete(IdStr);
