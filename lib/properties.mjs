    ...
    */
    constructor(context = new Context()) {
        if (privateBag.has(context)) {
            ...
            let { bag, event } = privateBag.get(context);
            let _context = context;
            if (!bag || !event) {
                _context = privateBag.get(context);
                ({ bag, event } = privateBag.get(_context));
            }
            const sharedBag = new Bag(_context);
            const sharedRaiseEvent = new RaiseEvent(_context);
