import { ComplexType, Container, Member, MemberParameter, MethodMember, PropertyMember, TypeMapper, randomUUID } from "../../registry.mjs";
...
export class ClassMember extends Member {
    ...
    */
    constructor(targetClass) {
        ...
        TypeMapper.register(targetClass);

        const typeMap = new TypeMapper(targetClass.name);

        super.update(targetClass.name, true, false, false, false, false, false, false, typeMap, []);
        
        let script = targetClass.toString();
        let match = /\/\*\*[\w\\r\\n\@\{\}\s\*\<\>\:\,\'\[\]\=\:]+\*\//.exec(script);
        const members = [];
        let prevId = null;
        let currentId = null;
        ...
        for(const member of members) {
            ...
            if (!member.isCtor && !member.isSetter) {
                ...
                if (returnTypeStr) {
                    try {
                        ...
                        member.typeMap = new TypeMapper(returnTypeStr);
                    } catch(error) {
                        console.error(error);
            ...
            if (member.isSetter || member.isMethod) {
                ...
                if (paramRegExMatch) {
                    ...
                    for(const paramName of params) {
                        ...
                        if (isJsDocObjectMatch) {
                            member.parameters[paramName].typeMap = new TypeMapper('object');
                        } else {
                            try {
                                member.parameters[paramName].typeMap = new TypeMapper(paramTypeStr);
                            } catch(error) {
                                console.error(error);
                            }
                        }
                ...
                for(const paramName of paramNames) {
                    ...
                    const parameterTypeMap = parameter.typeMap;
                    const parameterValue = parameter.value;
                    const field = {};
                    field[paramName] = parameterValue;
                    const memberParameter = new MemberParameter(field, parameterTypeMap);
                    memberParameters.push(memberParameter);
                }
            }
            if (member.isCtor || member.isMethod) {
                const methodMember = new MethodMember(member.name, member.isStatic, member.isCtor, memberParameters, member.typeMap);
                super.child = methodMember;
            } else if (member.isProperty) {
                if (member.isGetter) {
                    const propertyMember = new PropertyMember(member.name, member.isStatic,true, false, memberParameters, member.typeMap);
                    super.child = propertyMember;
                }
                if (member.isSetter) {
                    const propertyMember = new PropertyMember(member.name, member.isStatic, false, true, memberParameters, member.typeMap);
                    super.child = propertyMember;
                }
            }
        }
