...
const schemaFileNames = [
    ...
    'class.method.schema.json',
    'class.parameter.schema.json',
    'typedefinition.schema.json',
];
for (const schemaFileName of schemaFileNames) {
    const { $id, title, type, properties, required } = JSON.parse(readFileSync(join(schemasDirPath, schemaFileName)));
