...
export class Schema {
    constructor() {
        ...
        if (Bag.has(this._Id, secureContext)) {
            return Bag.get(this._Id , secureContext, targetClass);
        } else {
    ...
    */
    validate(verify) {
        ...
        const schemaTypeInfo = new TypeInfo(this._targetClass);
        const properties = schemaTypeInfo.members.filter(x => x.isGetterProperty);
        for (const prop of properties) {
            let typeInfo = this[prop.name];
            if (!(typeInfo instanceof TypeInfo)) {
                throw new Error(`${prop.name} did not return TypeInfo`);
            }
        }
