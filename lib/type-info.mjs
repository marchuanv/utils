...
export class TypeInfo {
    /**
     * @param { { name: String, type: Function, isStatic: Boolean } | { name: String, type: NULL, isStatic: Boolean  } | { name: String, type: UNDEFINED, isStatic: Boolean } } info
    */
    constructor(info = { name: UNKNOWN.name, type: UNKNOWN, isStatic: false }) {
        ...
        if (invalidType) {
            ...
            if (Bag.hasTypeInfo(info.name)) {
                return Bag.getTypeInfo(info.name);
            }
            const { type } = Reflection.getPrimitiveType(info.name);
            info.type = type;
            isPrimitiveType = true;
        } else {
            if (Bag.hasTypeInfo(info.type)) {
                return Bag.getTypeInfo(info.type);
            }
        ...
        if (info.type === undefined) {
            ...
            if (isPrimitiveType) {
                ...
                if (info.isStatic) {
                    info.defaultValue = new info.type();
                } else {
                    info.defaultValue = null;
                }
            }
        ...
        Bag.addTypeInfo(properties.type, this);
        Object.setPrototypeOf(properties, TypeInfo);
        Object.freeze(properties);
        Bag.set(this._Id, secureContext, this, typeInfoSchema);
