...
export class TypeDefinition {
    ...
    */
    static async register(config) {
        ...
        const scriptDirectoryPaths = [...new Set(config.map(x => x.scriptsDirPath))];
        for (const scriptDirectoryPath of scriptDirectoryPaths) {
            if (!existsSync(scriptDirectoryPath)) {
                throw new Error(`${scriptDirectoryPath} does not exist`);
            }
            walkDir(scriptDirectoryPath, (filePath) => {
                for (const { scriptFileName, targetClass } of config) {
                    const extName = extname(scriptFileName);
                    const _scriptFileName = scriptFileName.replace(extName, '');
                    const fileMatch = `${_scriptFileName}.interface.json`;
                    if (filePath.endsWith(fileMatch)) {
        ...
        if (transformedConfig.length !== config.length) {
            throw new Error('not all scripts were loaded.');
        }
        ...
        for (const { scriptFilePath, targetClass } of transformedConfig) {
            const typeName = targetClass.name;
            const classInterfaceFilePath = scriptFilePath;
