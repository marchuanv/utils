...
export class UUID {
    ...
    */
    constructor(str = null, secureContext = Bag.getSecureContext()) {
        if (new.target !== UUID) {
            throw new Error(`${UUID.name} is a sealed class.`);
        }
        if (secureContext === null || secureContext === undefined || !(secureContext instanceof SecureContext)) {
            throw new Error(`secure context is null, undefined or not an instance of a ${SecureContext.name}}`);
        }
        ...
        if (Bag.hasUUID(this.Id, secureContext)) {
            return Bag.getUUID(this.Id, secureContext );
        } else {
            Bag.setUUID(this, secureContext);
        }
        Object.freeze(this);
    }
    /**
